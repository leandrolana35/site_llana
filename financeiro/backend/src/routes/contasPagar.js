...
const buildWhere = (empresaId, query) => {
  const { status, pessoaId, dataInicio, dataFim, origem, busca } = query;
...
router.post('/', async (req, res) => {
  try {
    ...
    if (totalParcelas > 1) {
      ...
      const created = await prisma.contaPagar.createMany({ data: parcelas });
...
router.get('/resumo', async (req, res) => {
  try {
    ...
    const [abertos, vencidos, pagosMes, aVencerMes] = await prisma.$transaction([
    ...
    res.json({ abertos, vencidos, pagosMes, aVencerMes });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    ...
    if (contaBancariaId) {
      await prisma.contaBancaria.update({
        ...
        data: { saldoAtual: { decrement: vPago } },
      });
    }
